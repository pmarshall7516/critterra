...
export function DuelApp() {
  ...
  const moveDraftMember = (memberIndex: number, direction: -1 | 1) => {
    setDraft((current) => {
      if (!current) {
        return current;
      }
              ...
              <div className="duel-editor__content">
                ...
                {draft && (
                  <section className="duel-editor__form">
                    ...
                    <div className="duel-members">
                      <div className="duel-members__toolbar">
                        <h3>Squad Members ({draft.members.length}/8)</h3>
                        <button
                          type="button"
                          className="secondary"
                          disabled={draft.members.length >= 8}
                          onClick={() => {
                            const usedIds = new Set(draft.members.map((entry) => entry.critterId));
                      ...
                      {draft.members.map((member, memberIndex) => {
                        ...
                        const speciesIssues = validationIssuesByPath.get(`members.${memberIndex}.critterId`) ?? [];
                        const levelIssues = validationIssuesByPath.get(`members.${memberIndex}.level`) ?? [];
                        const skillIssues =
                          draftValidation?.issues
                            .filter((issue) => issue.path.startsWith(`members.${memberIndex}.equippedSkillIds`))
                            .map((issue) => issue.message) ?? [];
                        const equipmentIssues =
                          draftValidation?.issues
                            .filter((issue) => issue.path.startsWith(`members.${memberIndex}.equippedItems`))
                            .map((issue) => issue.message) ?? [];

                        return (
                          <article key={`draft-member-${memberIndex}`} className="duel-member-card">
                            <header className="duel-member-card__head">
                              ...
                              <div className="title-screen__actions">
                                <button
                                  ...
                                  onClick={() => moveDraftMember(memberIndex, -1)}
                                >
                                  Up
                                </button>
                                <button
                                  type="button"
                                  className="secondary"
                                  disabled={memberIndex >= draft.members.length - 1}
                                  onClick={() => moveDraftMember(memberIndex, 1)}
                                >
                                  Down
                                </button>
                            ...
                            <div className="duel-member-card__grid">
                              <label>
                                Species
                                <select
                                  ...
                                  onChange={(event) => {
                                    ...
                                    setDraft((current) => {
                                      ...
                                      return {
                                        ...current,
                                        members: current.members.map((entry, index) =>
                                          index === memberIndex
                                            ? {
                                                ...entry,
                                ...
                                >
                                  {catalogs.critters
                                    .filter((entry) => {
                                      ...
                                      return !draft.members.some((check, index) => index !== memberIndex && check.critterId === entry.id);
                                    })
                                    .map((entry) => (
                                      ...
                                      return {
                                        ...
                                        members: current.members.map((entry, index) =>
                                          index === memberIndex
                                            ? {
                                                ...entry,
                                                level: Number.isFinite(nextLevel)
                                                  ? Math.max(1, Math.min(maxLevel, nextLevel))
                                                  : 1,
                                              }
                                            : entry,
                                        ),
                              ...
                              <p className="admin-note">
                                Stats:{' '}
                                {derived
                                  ? `HP ${derived.effectiveStats.hp} | ATK ${derived.effectiveStats.attack} | DEF ${derived.effectiveStats.defense} | SPD ${derived.effectiveStats.speed}`
                                  : 'N/A'}
                              </p>
                              <p className="admin-note">Equip Slots Unlocked: {equipSlotCount}</p>
                            </div>

                            <section className="duel-member-card__skills">
                              <h5>Equipped Skills</h5>
                              <div className="duel-member-card__skill-grid">
                                {[0, 1, 2, 3].map((slotIndex) => {
                                  ...
                                  return (
                                    <label key={`member-${memberIndex}-skill-slot-${slotIndex}`}>
                                      Slot {slotIndex + 1}
                                      <select
                            ...
                            <section className="duel-member-card__equipment">
                              ...
                              <div className="duel-equip-slots">
                                {Array.from({ length: Math.max(0, equipSlotCount) }, (_, slotIndex) => {
                                  ...
                                  const occupyingItemDef = occupyingItem
                                    ? catalogIndexes.itemById.get(occupyingItem.itemId)
                                    : null;
                                  return (
                              ...
                              {member.equippedItems.map((equippedItem, itemIndex) => {
                                ...
                                return (
                                  <div key={`member-${memberIndex}-equipped-item-${itemIndex}`} className="duel-equip-item-row">
                                    <label>
                                      Item
                                      <select

...
import {
  ...
  buildSkillEffectBuffPercentById,
  computeBattleDamage,
...
interface BattleCritterState {
  ...
  defense: number;
  speed: number;
  /** Defense gained from positive equipment contributions; ignored on critical hits. */
  ...
  /** Additional crit chance (0..1) that applies to the next action only. */
  ...
  /** Normalized per-item equipment effect instances (allows same template with different values). */
...
interface BattlePostDamageResolution {
  ...
  defenderEffectIds: string[];
  defenderEffectBuffPercentById?: Record<string, number>;
  attackerCritChanceBuffEffectIds?: string[];
  attackerCritChanceBuffPercentById?: Record<string, number>;
  attackerCritChanceBuffPercent: number;
  attackerEffectSourceName?: string;
  healMessageAttackerName?: string;
  /** When set, credit this player critter for heal_with_skills (e.g. end-of-turn persistent heal from skill). */
  skillHealSourceCritterId?: number;
  /** When skillHealSourceCritterId is set, credit at most this much for heal_with_skills (defaults to healToAttacker). */
  skillHealAmount?: number;
  persistentHeals?: Array<{
...
export class GameRuntime {
  ...
  constructor(options?: RuntimeInitOptions) {
    ...
    if (storedWorldContent) {
      ...
      const effectTypeById = new Map(
        this.skillEffectLibrary.map((effect) => [effect.effect_id, effect.effect_type] as const),
      );
  ...
  /** RNG accessor that stays safe for test harnesses that bypass the constructor. */
...
private getStaticMapRenderCache(map: WorldMap): StaticMapRenderCache | null {
  ...
  public tryAdvanceCritter(critterId: number): boolean {
    ...
    // Any unlocked critter is implicitly seen.
    progress.seen = true;
    ...
    const equipmentEffects = this.getEquipmentEffectInstancesFromAnchors(equipmentState.anchors);
  ...
  public battleSelectSquadSlot(slotIndex: number): boolean {
    ...
    const previousCritterId =
      previousIndex !== null && battle.playerTeam[previousIndex]
        ? battle.playerTeam[previousIndex].critterId
        : null;
    ...
    const didUpdateSwapOut =
      previousCritterId !== null
        ? this.incrementSimpleCritterMissionProgress(previousCritterId, (mission) => mission.type === 'swap_out')
        : false;
    if (didUpdateSwapIn || didUpdateSwapOut) {
      this.markProgressDirty();
    }
    const previousName =
  ...
  private startNpcBattle(
    ...
    // Seeing an NPC's team should mark those critters as seen for the player.
    this.markCritterIdsSeen(opponentTeam.map((member) => member.critterId));
  ...
  private tryTriggerWalkEncounter(): void {
    ...
    if (this.getRng()() >= group.walkFrequency) {
      return;
    }
  ...
  private resolveWildEncounterDropRewards(
    ...
    for (const drop of drops) {
      ...
      if (dropChance <= 0 || this.getRng()() >= dropChance) {
        continue;
      }
  ...
  private startWildBattle(
    ...
    // Seeing a wild critter should mark it as seen for the player.
    this.markCritterSeen(critter.id);
    const playerTeam = this.buildPlayerBattleTeam();
    if (!this.hasBattleReadyCritter(playerTeam)) {
      this.handleEmptyPlayerBattleTeam();
      return;
  ...
  private startBattle(
    ...
    // Any critter the player battles against counts as "seen" for this save.
    this.markCritterIdsSeen(opponentTeam.map((member) => member.critterId));
  ...
  private resolveEncounterLevel(
    ...
    const index = Math.floor(this.getRng()() * levelsInRange.length);
  ...
  private buildWildBattleCritter(critter: CritterDefinition, levelOverride: number | null = null): BattleCritterState {
    ...
    const shuffledSkills = [...unlockedSkillIds].sort(() => this.getRng()() - 0.5);
  ...
  private resolvePlayerTurnAction(
    ...
    const opponentSkillSelection = this.selectOpponentBattleSkill(opponent);
    const playerPriority = this.resolveSkillPriority(playerSkill);
    const opponentPriority = this.resolveSkillPriority(opponentSkillSelection.skill);
    const playerEffectiveSpeed = this.getEffectiveBattleSpeed(player);
    const opponentEffectiveSpeed = this.getEffectiveBattleSpeed(opponent);
  ...
  private getOrderedEndTurnBattleCritters(
    battle: BattleRuntimeState,
  ): Array<{ team: 'player' | 'opponent'; critter: BattleCritterState }> {
    const ordered: Array<{ team: 'player' | 'opponent'; critter: BattleCritterState }> = [];
    const player = this.getActiveBattleCritter(battle, 'player');
    const opponent = this.getActiveBattleCritter(battle, 'opponent');
    if (player && !player.fainted && player.currentHp > 0) {
      ordered.push({ team: 'player', critter: player });
    }
    if (opponent && !opponent.fainted && opponent.currentHp > 0) {
      ordered.push({ team: 'opponent', critter: opponent });
    }
    ordered.sort((left, right) => {
      const speedDelta = this.getEffectiveBattleSpeed(right.critter) - this.getEffectiveBattleSpeed(left.critter);
      if (speedDelta !== 0) {
        return speedDelta;
      }
      return left.team === right.team ? 0 : left.team === 'player' ? -1 : 1;
    });
    return ordered;
  }
  ...
  private selectOpponentBattleSkill(
    attacker: Pick<BattleCritterState, 'equippedSkillIds'> | null,
  ): { skill: SkillDefinition; skillSlotIndex: number } {
    let skill = DEFAULT_TACKLE_SKILL;
    let skillSlotIndex = 0;
    if (!attacker?.equippedSkillIds) {
    ...
    const randomIndex = Math.floor(this.getRng()() * availableSkills.length);
  ...
  private executeBattleSkill(
    ...
    const { skill, skillSlotIndex } =
      attackingTeam === 'opponent'
        ? this.selectOpponentBattleSkill(attacker)
        : { skill: DEFAULT_TACKLE_SKILL, skillSlotIndex: 0 };
  ...
  private executeBattleSkillWithSkill(
    ...
    const defenderTeam: 'player' | 'opponent' = attackingTeam === 'player' ? 'opponent' : 'player';

    const applyDamageDefender: 'player' | 'opponent' = defenderTeam;
    ...
    const appliedAttachments = resolvedAttachments.filter((attachment) =>
      attachment.procChance >= 1 || this.getRng()() < attachment.procChance,
    );
    const attachmentResolution = resolveAppliedSkillAttachments(appliedAttachments, this.skillEffectLookupById);
    const persistentHealResolutions = this.buildPersistentHealResolutions(
      attachmentResolution.persistentHealAttachments,
      skill.skill_name,
      skill.type === 'support' ? attackingTeam : defenderTeam,
      attackingTeam === 'player' && battle.playerActiveIndex != null
        ? battle.playerTeam[battle.playerActiveIndex]?.critterId
        : undefined,
    );

    if (skill.type === 'support') {
      const requestedHeal = resolveRequestedSkillHealAmount(skill, attacker.maxHp, 0);
      const plannedRecoil = resolveRequestedSkillRecoilAmount(
        attachmentResolution.recoilAttachments,
        attacker.maxHp,
        0,
      );
      const followUpEvents = this.buildBattleResolutionNarrationEvents(
        ...
        requestedHeal,
        plannedRecoil,
        attachmentResolution.attackerEffectAttachments.map((attachment) => attachment.effectId),
        buildSkillEffectBuffPercentById(attachmentResolution.attackerEffectAttachments),
        attachmentResolution.defenderEffectAttachments.map((attachment) => attachment.effectId),
        buildSkillEffectBuffPercentById(attachmentResolution.defenderEffectAttachments),
        attachmentResolution.attackerCritChanceBuffAttachments.map((attachment) => attachment.effectId),
        buildSkillEffectBuffPercentById(attachmentResolution.attackerCritChanceBuffAttachments),
        attachmentResolution.attackerCritChanceBuffPercent,
        skill.skill_name,
    ...
    const equipmentCritChanceBonus = this.getEquipmentCritChanceBonus(attacker);
    const consumedCritBonus = clamp(
      consumePendingCritChanceBonus(attacker, this.skillEffectLookupById) + equipmentCritChanceBonus,
      0,
      1,
    );

    const damageResult = computeBattleDamage({
      attacker: {
    ...
    const { damage, damageDealt, isCrit, typeMult } = damageResult;
    const plannedHeal = resolveRequestedSkillHealAmount(skill, attacker.maxHp, damageDealt);
    const plannedRecoil = resolveRequestedSkillRecoilAmount(
      attachmentResolution.recoilAttachments,
      attacker.maxHp,
      damageDealt,
    ...
    const message = `${attackPrefix} dealt ${damage} damage.`;
      ...
      narrationEvents: [
        {
          ...
          followUpEvents: [
            ...this.buildBattleDamageOutcomeNarrationEvents(isCrit, typeMult, defenderGuarded, guardSucceeded),
            ...followUpEvents,
            ...statusEvents,
          ],
  ...
  private buildBattleDamageOutcomeNarrationEvents(
    isCrit: boolean,
    typeMult: number,
    defenderGuarded: boolean,
    guardSucceeded: boolean,
    ...
    if (isCrit) {
      events.push({
        message: 'It was a crit!',
        attacker: null,
      });
    }
    if (typeMult >= 2) {
      events.push({ message: "It's super effective!", attacker: null });
    } else if (typeMult >= 1.1) {
      events.push({ message: "It's effective!", attacker: null });
    } else if (typeMult < 1 && typeMult > 0) {
      events.push({ message: "It's not very effective...", attacker: null });
    } else if (typeMult === 0) {
      events.push({ message: 'It had no effect.', attacker: null });
    }
    if (defenderGuarded && guardSucceeded) {
      events.push({
        message: 'Guard reduced the damage.',
        attacker: null,
      });
    }
    return events;
  ...
  private buildPersistentHealResolutions(
    attachments: SkillEffectAttachment[],
    sourceName: string,
    targetTeam: 'player' | 'opponent',
    playerSourceCritterId?: number,
  ): NonNullable<BattlePostDamageResolution['persistentHeals']> {
    const resolved: NonNullable<BattlePostDamageResolution['persistentHeals']> = [];
    for (const attachment of attachments) {
      const effectId = typeof attachment.effectId === 'string' ? attachment.effectId.trim() : '';
      if (!effectId) {
        continue;
      }
      ...
      resolved.push({
        ...
        ...(playerSourceCritterId != null && { playerSourceCritterId }),
      });
    }
    return resolved;
  }

  private applySkillEffectsToCritter(
  ...
  private applyPersistentHealStateToCritter(
    target: BattleCritterState,
    persistentHeal: NonNullable<BattlePostDamageResolution['persistentHeals']>[number],
  ): void {
    this.clearPersistentHealStateForCritter(target);
    target.persistentHeal = {
      effectId: persistentHeal.effectId,
      sourceName: persistentHeal.sourceName,
      mode: persistentHeal.mode,
      value: persistentHeal.value,
      remainingTurns: persistentHeal.remainingTurns,
      ...(persistentHeal.playerSourceCritterId != null && { playerSourceCritterId: persistentHeal.playerSourceCritterId }),
    };
    if (!target.activeEffectIds.includes(persistentHeal.effectId)) {
      target.activeEffectIds.push(persistentHeal.effectId);
    }
    target.activeEffectSourceById[persistentHeal.effectId] = persistentHeal.sourceName;
    target.activeEffectValueById = target.activeEffectValueById ?? {};
    target.activeEffectValueById[persistentHeal.effectId] = persistentHeal.value;
  }

  private clearPersistentHealStateForCritter(target: BattleCritterState): void {
    if (!target.persistentHeal) {
      return;
    }
    const effectId = target.persistentHeal.effectId;
    target.persistentHeal = null;
    if (!effectId) {
      return;
    }
    target.activeEffectIds = target.activeEffectIds.filter((entry) => entry !== effectId);
    delete target.activeEffectSourceById[effectId];
  ...
  private applyBattlePostDamageResolution(
    ...
    if (actualHeal > 0) {
      const critterIdToCredit =
        resolution.skillHealSourceCritterId ??
        (resolution.attackingTeam === 'player' ? attacker.critterId : undefined);
      if (critterIdToCredit != null) {
        const amountToCredit =
          resolution.skillHealSourceCritterId != null
            ? (resolution.skillHealAmount ?? actualHeal)
            : actualHeal;
        this.advanceHealWithSkillsMissions(critterIdToCredit, amountToCredit);
      }
    }
    if (resolution.persistentHeals && resolution.persistentHeals.length > 0) {
      for (const persistentHeal of resolution.persistentHeals) {
        const target = this.getActiveBattleCritter(battle, persistentHeal.targetTeam);
        if (!target || target.fainted || target.currentHp <= 0) {
          continue;
        }
        this.applyPersistentHealStateToCritter(target, persistentHeal);
      }
    }
    if (resolution.attackerEffectIds.length > 0) {
      this.applySkillEffectsToCritter(
        attacker,
    ...
    if (
      ...
      resolution.defenderEffectIds.length > 0
    ) {
      this.applySkillEffectsToCritter(
        defender,
        resolution.defenderEffectIds,
        resolution.defenderEffectBuffPercentById,
        resolution.attackerEffectSourceName,
      );
    }
    if (resolution.attackerCritChanceBuffPercent > 0) {
      const effectIds = resolution.attackerCritChanceBuffEffectIds ?? [];
      const effectValueById = resolution.attackerCritChanceBuffPercentById ?? {};
      if (effectIds.length > 0) {
        for (const effectId of effectIds) {
          const buffPercent = clamp(
            effectValueById[effectId] ?? this.skillEffectLookupById[effectId]?.buffPercent ?? 0,
            0,
            1,
          );
          applyPendingCritChanceBuff(attacker, effectId, resolution.attackerEffectSourceName ?? '', buffPercent);
        }
      } else {
        attacker.pendingCritChanceBonus = clamp(
          attacker.pendingCritChanceBonus + resolution.attackerCritChanceBuffPercent,
          0,
          1,
        );
      }
    }
    const recoilDamage = Math.max(0, Math.floor(resolution.recoilToAttacker));
    const priorHp = attacker.currentHp;
    if (recoilDamage > 0) {
  ...
  private buildBattleResolutionNarrationEvents(
    ...
    defenderEffectIds: string[],
    defenderEffectBuffPercentById: Record<string, number> | undefined,
    attackerCritChanceBuffEffectIds: string[],
    attackerCritChanceBuffPercentById: Record<string, number> | undefined,
    attackerCritChanceBuffPercent: number,
    attackerEffectSourceName?: string,
    persistentHeals?: BattlePostDamageResolution['persistentHeals'],
  ): BattleNarrationEvent[] {
    const events: BattleNarrationEvent[] = [];
    ...
    if (healToAttacker > 0) {
      events.push({
        ...
        postDamageResolution: {
          ...
          attackerEffectIds: [],
          defenderEffectIds: [],
          attackerCritChanceBuffEffectIds: [],
          attackerCritChanceBuffPercent: 0,
          healMessageAttackerName: attackerName,
        },
      });
    }

    if (persistentHeals && persistentHeals.length > 0) {
      for (const persistentHeal of persistentHeals) {
        const targetName =
          persistentHeal.targetTeam === attackingTeam ? attackerName : defenderName;
        events.push({
          message:
            persistentHeal.targetTeam === 'player'
              ? `Your ${targetName} gained end-of-turn healing for ${persistentHeal.remainingTurns} turns.`
              : `${targetName} gained end-of-turn healing for ${persistentHeal.remainingTurns} turns.`,
          attacker: null,
          postDamageResolution: {
            attackingTeam,
            defendingTeam,
            ...
            attackerCritChanceBuffEffectIds: [],
            attackerCritChanceBuffPercent: 0,
            persistentHeals: [persistentHeal],
          },
        });
      }
    }
    ...
    if (defenderEffectIds.length > 0) {
      events.push({
        message: this.formatBattleEffectActivationMessage(defenderName, defenderEffectIds),
        attacker: null,
        postDamageResolution: {
          attackingTeam,
          defendingTeam,
          ...
          attackerCritChanceBuffEffectIds: [],
          attackerCritChanceBuffPercent: 0,
          defenderEffectBuffPercentById,
          attackerEffectSourceName,
        },
      });
    }

    if (attackerCritChanceBuffPercent > 0) {
      events.push({
        message: `${attackerName}'s critical hit chance rose.`,
        attacker: null,
        postDamageResolution: {
          attackingTeam,
          defendingTeam,
          ...
          defenderEffectIds: [],
          attackerCritChanceBuffEffectIds,
          attackerCritChanceBuffPercentById,
    ...
    if (recoilToAttacker > 0) {
      events.push({
        message: `${attackerName} took ${Math.floor(recoilToAttacker)} recoil damage.`,
        attacker: null,
        postDamageResolution: {
          attackingTeam,
          defendingTeam,
  ...
  private buildEndTurnHealingNarrationEvents(battle: BattleRuntimeState): BattleNarrationEvent[] {
    const events: BattleNarrationEvent[] = [];
    for (const { team, critter } of this.getOrderedEndTurnBattleCritters(battle)) {
      this.clearFlinchForCritter(critter);
    }
    for (const { team, critter } of this.getOrderedEndTurnBattleCritters(battle)) {
      let projectedHp = critter.currentHp;
      if (critter.fainted || projectedHp <= 0) {
        continue;
      }
      if (critter.persistentStatus?.kind === 'toxic') {
        ...
        if (toxicDamage > 0 && projectedHp > 0) {
          const actualDamage = Math.max(0, Math.min(projectedHp, toxicDamage));
          if (actualDamage > 0) {
            events.push({
              message:
                team === 'player'
              ...
              attacker: null,
              applyDamageDefender: team,
              applyDamageAmount: actualDamage,
              pendingKnockoutResolution:
                projectedHp <= actualDamage
                  ? {
                      defenderTeam: team,
                      playerAttackerCritterId: null,
                    }
                  : undefined,
            });
          }
          projectedHp = Math.max(0, projectedHp - actualDamage);
        }
        if (critter.persistentStatus?.kind === 'toxic') {
      ...
      let skillHealSourceCritterId: number | undefined;
      const healSourceNames: string[] = [];
      if (critter.persistentHeal) {
        skillHealRequested = this.resolvePersistentHealAmount(critter, critter.persistentHeal);
        skillHealSourceCritterId = critter.persistentHeal.playerSourceCritterId;
        requestedHeal += skillHealRequested;
      ...
      for (const equipmentHeal of this.getEquipmentPersistentHealConfigs(critter)) {
        requestedHeal += this.resolvePersistentHealAmount(critter, equipmentHeal);
        if (equipmentHeal.sourceName.trim()) {
          healSourceNames.push(equipmentHeal.sourceName.trim());
        }
      }
      const actualHeal = Math.min(requestedHeal, Math.max(0, critter.maxHp - projectedHp));
      if (actualHeal <= 0) {
        continue;
      }
      const skillHealAmount =
        skillHealSourceCritterId != null ? Math.min(skillHealRequested, actualHeal) : undefined;
      const uniqueSourceNames = healSourceNames.filter((entry, index, values) => values.indexOf(entry) === index);
      const sourceLabel = uniqueSourceNames.length > 0 ? uniqueSourceNames.join(', ') : 'end-of-turn effects';
      events.push({
        message:
          team === 'player'
            ? `Your ${critter.name} was healed +${actualHeal} HP by ${sourceLabel}.`
            : `${critter.name} was healed +${actualHeal} HP by ${sourceLabel}.`,
        attacker: null,
        postDamageResolution: {
          attackingTeam: team,
          defendingTeam: team === 'player' ? 'opponent' : 'player',
          healToAttacker: actualHeal,
          recoilToAttacker: 0,
          attackerEffectIds: [],
          defenderEffectIds: [],
          attackerCritChanceBuffPercent: 0,
          ...(skillHealSourceCritterId != null && {
            skillHealSourceCritterId,
            ...(skillHealAmount != null && { skillHealAmount }),
          }),
        },
      });
    }
  ...
  private getEquipmentPersistentHealConfigs(
    critter: BattleCritterState,
    ...
    for (const effect of critter.equipmentEffectInstances ?? []) {
      ...
      const value = mode === 'flat'
        ...
        : clamp(effect.persistentHealValue ?? 0, 0, 1);
      if (value <= 0) {
        continue;
      }
      configs.push({
        mode,
        value,
        sourceName: (critter.equipmentEffectSourceById[effect.effectId]?.[0] ?? effect.effectId).trim(),
      });
    }
    return configs;
  }

  private getEquipmentCritChanceBonus(
    critter: Pick<BattleCritterState, 'equipmentEffectInstances'>,
  ): number {
    let total = 0;
    for (const effect of critter.equipmentEffectInstances ?? []) {
      ...
      total += clamp(effect.critChanceBonus ?? 0, 0, 1);
    }
    return clamp(total, 0, 1);
  }

  private resolvePersistentHealAmount(
    critter: Pick<BattleCritterState, 'maxHp'>,
    persistentHeal: Pick<NonNullable<BattleCritterState['persistentHeal']>, 'mode' | 'value'>,
  ): number {
    if (persistentHeal.mode === 'flat') {
      return Math.max(1, Math.floor(persistentHeal.value));
    }
    return Math.max(0, Math.floor(critter.maxHp * clamp(persistentHeal.value, 0, 1)));
  }
  ...
  private formatBattleSupportSkillMessage(
    battle: BattleRuntimeState,
    attackingTeam: 'player' | 'opponent',
    attackerName: string,
    skillName: string,
    ...
    if (attackingTeam === 'player') {
      return `Your ${attackerName} used ${skillName}.`;
    }
    if (battle.source.type === 'wild') {
      return `The wild ${attackerName} used ${skillName}.`;
    }
    return `${battle.source.label}'s ${attackerName} used ${skillName}.`;
  }
      ...
      if (next.postDamageResolution.healMessageAttackerName) {
        next.message =
          resolutionResult.actualHeal > 0
            ? `${next.postDamageResolution.healMessageAttackerName} restored ${resolutionResult.actualHeal} HP.`
            : `${next.postDamageResolution.healMessageAttackerName} could not restore any HP.`;
        battle.logLine = next.message;
      }
      if (resolutionResult.attackerJustFainted) {
        battle.narrationQueue.unshift(
          ...this.buildBattleKnockoutNarrationEvents(
            battle,
            next.postDamageResolution.attackingTeam,
            null,
          ),
        );
      }
  ...
  private mapBattleTeamEntryToSnapshot(entry: BattleCritterState): RuntimeBattleSnapshot['playerTeam'][number] {
    ...
    const skillEffectAttachmentById: Record<string, SkillEffectAttachment> = {};
    if (Array.isArray(entry.equippedSkillIds)) {
      for (const skillId of entry.equippedSkillIds) {
        ...
        for (const attachment of effectAttachments) {
          ...
          skillEffectAttachmentById[effectId] = { ...attachment };
          ...
          skillEffectBuffPercentById[effectId] =
            (skillEffectBuffPercentById[effectId] ?? 0) + buffPercent;
        }
      }
    }

    const equipmentInstancesById = groupEquipmentEffectInstancesById(entry.equipmentEffectInstances ?? []);
    const activeSkillEffectIds = [...(entry.activeEffectIds ?? [])]
      .filter((id, index, values) => values.indexOf(id) === index);
    const activeEquipmentEffectIds = [...(entry.equipmentEffectIds ?? [])]
      .filter((id, index, values) => values.indexOf(id) === index);
    const activeEffectIds = [...activeEquipmentEffectIds, ...activeSkillEffectIds]
      .filter((id, index, values) => values.indexOf(id) === index);
    const activeEffectIconUrls: string[] = [];
    const activeEffectDescriptions: string[] = [];
    for (const id of activeSkillEffectIds) {
        ...
        if (typeof url === 'string' && url.length > 0) {
          ...
          const buffPercent = typeof appliedBuffPercent === 'number'
            ...
            : typeof skillEffect.buffPercent === 'number'
              ...
              : 0.1;
          const baseAttachment = skillEffectAttachmentById[id] ?? {
            effectId: id,
            procChance: 1,
            buffPercent,
          };
          const attachmentForTooltip =
            entry.persistentHeal?.effectId === id
              ? {
                ...baseAttachment,
                persistentHealMode: entry.persistentHeal.mode,
                persistentHealValue: entry.persistentHeal.value,
                persistentHealDurationTurns: entry.persistentHeal.remainingTurns,
              }
              : {
                ...baseAttachment,
                buffPercent,
              };
          const effectDescription = formatSkillEffectDescription(skillEffect, {
            ...attachmentForTooltip,
            effectId: id,
            procChance: 1,
          });
          const stackSummary = this.summarizeSkillEffectStackTotal(skillEffect, buffPercent);
          const tooltipBody = [effectDescription.trim(), stackSummary]
            .filter((text): text is string => typeof text === 'string' && text.length > 0)
            .join('\n');
          const sourceNames = [
            entry.persistentHeal?.effectId === id ? entry.persistentHeal.sourceName : '',
            entry.activeEffectSourceById[id] ?? '',
            ...(skillEffectSourcesById[id] ?? []),
          ].filter((source): source is string => typeof source === 'string' && source.trim().length > 0);
          activeEffectDescriptions.push(
            this.formatBattleEffectTooltip(tooltipBody, sourceNames),
          );
        }
      }
    }
    for (const id of activeEquipmentEffectIds) {
      ...
      if (typeof url === 'string' && url.length > 0) {
        ...
        activeEffectDescriptions.push(
          this.formatBattleEffectTooltip(
            this.summarizeEquipmentEffect(equipmentEffect, equipmentInstancesById.get(id) ?? []),
    ...
    if (entry.persistentStatus) {
      ...
      if (statusPresentation) {
        ...
        const statusEffectId = entry.persistentStatus.effectId?.trim() ?? '';
        const statusIconUrl =
          this.skillEffectLookupById[statusEffectId]?.iconUrl ||
          this.equipmentEffectLookupById[statusEffectId]?.iconUrl ||
          statusPresentation.iconUrl;
        activeEffectIconUrls.push(statusIconUrl);
        activeEffectDescriptions.push(
          this.formatBattleEffectTooltip(
    ...
    if (entry.flinch) {
      ...
      const flinchEffectId = entry.flinch.effectId?.trim() ?? '';
      const flinchIconUrl =
        this.skillEffectLookupById[flinchEffectId]?.iconUrl ||
        this.equipmentEffectLookupById[flinchEffectId]?.iconUrl ||
        flinchPresentation.iconUrl;
      activeEffectIconUrls.push(flinchIconUrl);
      activeEffectDescriptions.push(
        this.formatBattleEffectTooltip(
    ...
    return {
      ...
      attack: this.getEffectiveBattleStat(entry.attack, entry.attackModifier),
      defense: this.getEffectiveBattleStat(entry.defense, entry.defenseModifier),
      speed: Math.max(1, Math.round(this.getEffectiveBattleSpeed(entry))),
  ...
  private getEffectiveBattleStat(baseStat: number, modifier: number): number {
    return Math.max(1, Math.round(baseStat * Math.max(MIN_BATTLE_STAT_MODIFIER, modifier)));
  }

  private summarizeSkillEffectStackTotal(
    effect: Pick<SkillEffectDefinition, 'effect_type'>,
    stackedBuffPercent: number,
  ): string | null {
    if (
    ...
    if (!Number.isFinite(stackedBuffPercent) || Math.abs(stackedBuffPercent) <= 0) {
      return null;
    }
    ...
    const magnitude = Math.round(Math.abs(stackedBuffPercent) * 100);
    if (magnitude <= 0) {
      return null;
    }
    if (statLabel) {
  ...
  private getShopPromptSnapshot(): RuntimeSnapshot['story']['shopPrompt'] {
    ...
    const entries = shop.entries
      .map((entry) => {
        ...
        const affordability = this.getShopEntryAffordability(entry);
        if (critter) {
          seenCritterIds.push(critter.id);
        }
        return {
          entryId: entry.id,
    ...
    if (seenCritterIds.length > 0) {
      this.markCritterIdsSeen(seenCritterIds);
    }
  ...
  private openStarterSelection(): void {
    ...
    // Seeing all starter options should mark them as seen.
    this.markCritterIdsSeen(optionCritterIds);
    this.heldDirections = [];
    this.moveStep = null;
  }
  ...
  /** Mark a single critter as seen in the current save, if present. */
  private markCritterSeen(critterId: number): void {
    if (!Number.isFinite(critterId) || critterId <= 0) {
      return;
    }
    const entry = this.playerCritterProgress.collection.find((item) => item.critterId === critterId);
  ...
  /** Mark multiple critters as seen in the current save. */
  private markCritterIdsSeen(critterIds: number[]): void {
    let changed = false;
    for (const rawId of critterIds) {
      if (!Number.isFinite(rawId) || rawId <= 0) {
        continue;
      }
      const entry = this.playerCritterProgress.collection.find((item) => item.critterId === rawId);
      if (!entry || entry.seen === true) {
        continue;
      }
      entry.seen = true;
  ...
  private getLockedKnockoutTargetContext(
    ...
    const critter = this.critterLookup[critterId];
    const progress = this.playerCritterProgress.collection.find((entry) => entry.critterId === critterId);
    if (!critter || !progress || progress.unlocked || progress.seen !== true) {
      return null;
    }
  ...
  private advanceHealWithSkillsMissions(critterId: number, amount: number): boolean {
    ...
    for (const mission of levelRow.missions) {
      ...
      const key = missionProgressKey(levelRow.level, mission.id);
      const currentValue = Math.max(0, progress.missionProgress[key] ?? 0);
      const nextValue = Math.min(mission.targetValue, currentValue + amount);
      if (nextValue <= currentValue) {
        continue;
      }
      progress.missionProgress[key] = nextValue;
  ...
  private advanceLandCriticalHitsMissions(critterId: number, count: number): boolean {
    ...
    for (const mission of levelRow.missions) {
      ...
      const key = missionProgressKey(levelRow.level, mission.id);
      const currentValue = Math.max(0, progress.missionProgress[key] ?? 0);
      const nextValue = Math.min(mission.targetValue, currentValue + count);
      if (nextValue <= currentValue) {
        continue;
      }
      progress.missionProgress[key] = nextValue;
  ...
  private getEquipmentEffectIdsForItem(item: GameItemDefinition): string[] {
    return resolveEquipmentEffectIdsForItem(item, this.equipmentEffectLookupById);
  }

  private getEquipmentEffectInstancesForItem(item: GameItemDefinition): EquipmentEffectInstance[] {
    return resolveEquipmentEffectInstancesForItem(item, this.equipmentEffectLookupById);
  }
  ...
  private getEquipmentEffectInstancesFromAnchors(anchors: EquippedEquipmentAnchor[]): EquipmentEffectInstance[] {
    const instances: EquipmentEffectInstance[] = [];
    for (const anchor of anchors) {
      const item = this.getEquipmentItemById(anchor.itemId);
      if (!item) {
        continue;
      }
      instances.push(...this.getEquipmentEffectInstancesForItem(item));
    }
    return instances;
  }
  ...
  private applyEquipmentEffectsToStats(
    ...
    for (const effect of effects) {
      ...
      if (effect.mode === 'flat') {
        flatByStat[stat] += value;
      } else {
        percentByStat[stat] += value;
      }
    }
    return {
  ...
  private summarizeEquipmentEffect(
    ...
    if (effect.persistentHeal) {
      const persistentLine = effect.persistentHeal.mode === 'flat'
        ? `End turn heal +${Math.max(1, Math.floor(effect.persistentHeal.value))} HP while equipped`
        : `End turn heal +${Math.round(clamp(effect.persistentHeal.value, 0, 1) * 100)}% max HP while equipped`;
      parts.push(persistentLine);
    }
    if (parts.length <= 0) {
  ...
  private buildEquipmentSlotSnapshot(
    ...
    const effectInstances = item ? this.getEquipmentEffectInstancesForItem(item) : [];
    const groupedInstances = groupEquipmentEffectInstancesById(effectInstances);
    const effectIconUrls: string[] = [];
    const effectDescriptions: string[] = [];
    for (const [effectId, instances] of groupedInstances.entries()) {
      const effect = this.equipmentEffectLookupById[effectId];
      if (!effect) {
        continue;
      }
      if (typeof effect.iconUrl === 'string' && effect.iconUrl.trim().length > 0) {
        effectIconUrls.push(effect.iconUrl);
      }
      const summary = this.summarizeEquipmentEffect(effect, instances);
      if (summary.trim().length > 0) {
        effectDescriptions.push(summary);
      }
    }
    return {
      itemId: slot.itemId,
  ...
  private getCritterSummary(): RuntimeSnapshot['critters'] {
    ...
    const collection = this.critterDatabase.map((critter) => {
      ...
      const seen = progress?.seen === true || unlocked;
...
function describeItemEffect(item: GameItemDefinition): string {
  ...
  if (item.effectType === 'equip_effect' || item.effectType === 'equip_stub') {
    const effect = item.effectConfig as { equipSize?: number; equipmentEffectIds?: string[]; equipmentEffectAttachments?: Array<{ effectId?: string }> };
    const equipSize =
      typeof effect.equipSize === 'number' && Number.isFinite(effect.equipSize)
        ? Math.max(1, Math.floor(effect.equipSize))
        : 1;
    const effectCountFromAttachments = Array.isArray(effect.equipmentEffectAttachments)
      ? effect.equipmentEffectAttachments
          .filter((entry) => entry && typeof entry.effectId === 'string' && entry.effectId.trim().length > 0)
          .length
      : 0;
    const effectCountFromIds = Array.isArray(effect.equipmentEffectIds) ? effect.equipmentEffectIds.length : 0;
    const effectCount = Math.max(effectCountFromAttachments, effectCountFromIds);
  ...
  return {
    ...
    postDamageResolution: event.postDamageResolution
      ? {
          ...
          defenderEffectIds: [...event.postDamageResolution.defenderEffectIds],
          attackerEffectBuffPercentById: event.postDamageResolution.attackerEffectBuffPercentById
            ...
            : undefined,
          defenderEffectBuffPercentById: event.postDamageResolution.defenderEffectBuffPercentById
            ? { ...event.postDamageResolution.defenderEffectBuffPercentById }
            : undefined,
          persistentHeals: event.postDamageResolution.persistentHeals
            ? event.postDamageResolution.persistentHeals.map((entry) => ({ ...entry }))
            : undefined,
        }
      : undefined,
...
function formatSkillEffectDescription(
  ...
  if (effectDescription) {
    ...
    const persistentModeLabel = attachment.persistentHealMode === 'flat' ? 'HP' : 'max HP';
